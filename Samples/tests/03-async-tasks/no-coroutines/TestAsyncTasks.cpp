#include "tests/03-async-tasks/no-coroutines/TestAsyncTasks.h"
...
std::vector<std::string> Demo03AsyncTasksNoCoroutines::collect_unmarshalled_thread_ids() {
    return no_coroutines::collect_unmarshalled_thread_ids();
}

std::vector<std::string> Demo03AsyncTasksNoCoroutines::collect_marshalled_thread_ids(EventLoop &loop) {
