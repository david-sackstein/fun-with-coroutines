#include "tests/3. async_tasks/coroutines/Demo3AsyncTasks.h"

#include <set>

TEST_F(Demo3AsyncTasksCoroutines, UnmarshalledSample) {
    const std::vector<std::string> thread_ids = collect_unmarshalled_thread_ids();
    ASSERT_EQ(thread_ids.size(), 3U);
    EXPECT_GT(std::set<std::string>(thread_ids.begin(), thread_ids.end()).size(), 1U);
}

TEST_F(Demo3AsyncTasksCoroutines, MarshalledSample) {
    EventLoop loop;
